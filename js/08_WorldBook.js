...
(function () {
    ...
    function entryCardHTML(e, dim) {
       ...
       function loadEntitiesDirect(cb) {
        try {
            var req = indexedDB.open('HomeChatDB', 1);
            req.onsuccess = function(e) {
                var db = e.target.result;
                if (!db.objectStoreNames.contains('kv')) { db.close(); cb([]); return; }
                var tx = db.transaction('kv', 'readonly');
                var store = tx.objectStore('kv');
                var getReq = store.get('groups');
                getReq.onsuccess = function(ev) {
                    var groups = ev.target.result || [];
                    db.close();
                    var ents = [];
                    groups.forEach(function(g) {
                        if (g.members) {
                            g.members.forEach(function(c) {
                                if (!ents.find(function(x) { return x.id === String(c.id); })) {
                                    ents.push({
                                        id: String(c.id),
                                        name: c.real || c.nm || '未命名',
                                        nickname: c.note || c.nm || '未命名',
                                        avatar: c.avaImg || ''
                                    });
                                }
                            });
